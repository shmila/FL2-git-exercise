...
class VGG(object):
    ...
    def __init__(self,pre_clean=True):
        ...
        pool5 = tf.layers.max_pooling2d(self.layers[-1], 2, 2, padding='same', name='pool5')
        self.layers.append(pool5)

        fc1 = tf.layers.conv2d(
            pool5, 512, 7, name='fc1', activation=tf.nn.relu, padding='same', kernel_regularizer=self.regularizer
        )
        dp1 = tf.layers.dropout(fc1, rate=0.5, name='drop1')
        fc2 = tf.layers.conv2d(
            dp1, 512, 1, name='fc2', activation=tf.nn.relu, padding='same', kernel_regularizer=self.regularizer
        )
        g_pool = tf.reduce_mean(fc2, [1, 2], keep_dims=True, name='global_pool')
        dp2 = tf.layers.dropout(g_pool, rate=0.5, name='drop2')
        output = tf.layers.conv2d(dp2, self.num_classes, [1, 1], activation=None)
